...
class HeatEquationPlotter(AbstractPlotter):
    def __init__(self, json_data_path, output_path):
        ...
        if "data" in self.data:
            ...
            for frame_data in self.data["data"]:
                ...
                field_clean = np.where(field == None, np.nan, field).astype(float)
                self.frames.append({
    ...
    def plot(self):
        ...
        # НОВОЕ: границы выреза (пунктиром)
        ...
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_xlim(0, 3)  # ИСПРАВЛЕНО
        ax.set_ylim(0, 3)  # ИСПРАВЛЕНО
        ax.set_aspect('equal')
        ...
        def update(frame_idx):
            ...
            new_contour = ax.contourf(X, Y, frame['field'], 
                                      levels=50, cmap='hot', vmin=vmin, vmax=vmax)
